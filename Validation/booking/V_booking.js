...
export const book_a_service_validator = (req, res, next) => {
  const schema = Joi.object({
    ...
    location: Joi.string().min(20).max(120).required().label("Location"),
...
export const get_booking_history_by_id_validator = (req, res, next) => {
  const schema = Joi.object({
    limit: Joi.number().greateer(0).required().label("Limit"),
    offset: Joi.number().greater(0).required().label("Offset"),
  });
  validation_helper(req, res, next, schema);
};
